...
async function crearPagoNormal(monto, moneda, plan, datosRestaurante, tipoTarjeta, cuotas, esPagoPrueba = false) {
  try {
      ...
      items: [
        {
          ...
          description: esPagoPrueba ? `PAGO DE PRUEBA - Sistema de gestión para restaurantes (${plan || 'anual'})` : `Sistema de gestión para restaurantes - ${plan || 'anual'}`,
        }
      ],
      payer: {
    ...
    if (esPagoPrueba) {
      console.log('🔧 CREANDO PAGO DE PRUEBA - Monto: 0.2 centavos ARS');
    }
