...
function PagoPageContent() {
  ...
  const handleMercadoPago = async (esPrueba = false) => {
    setProcesando(true)
    ...
    try {
      ...
      if (data.tipo === 'dividido') {
        ...
        if (esPrueba) {
          alert('🔧 PAGO DE PRUEBA - Solo 0.2 centavos ARS\n\nTe redirigiremos a Mercado Pago para completar el pago de prueba.')
        }
  ...
  const handlePagoPrueba = async () => {
    ...
    try {
      ...
      console.log('Datos del restaurante desde localStorage:', datosRestaurante)
      console.log('Campos disponibles:', Object.keys(datosRestaurante))
      
      const response = await fetch('/api/pago-prueba-firestore', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (data.success) {
        alert(`¡Pago de prueba exitoso!\n\nRestaurante ID: ${data.restaurantId}\nCódigo Activación ID: ${data.codigoActivacionId}\nPago ID: ${data.paymentId}\nCódigo de Activación: ${data.codigoActivacion}\n\n✅ Datos guardados en Firestore\n✅ Colección: restaurantes (ID = código de activación)\n✅ Colección: codigosactivacion (ID = código de activación)\n✅ 16 subcolecciones creadas con documentos específicos:\n   • CajaRegistradora → caja_principal\n   • Dinero → dinero_actual\n   • Mpagos → configuracion\n   • ServiciosComercio → servicios_basicos\n   • configuracion → notificaciones\n   • tables → mesa2-mesa9\n   • users → (vacío - el cliente creará usuarios)\n✅ Restaurante activado exitosamente`)
      } else {
          ...
          <div className="grid lg:grid-cols-2 gap-8">
            ...
            >
              ...
              <div className="space-y-4">
                ...
                {/* Mercado Pago de Prueba */}
                <motion.button
                  onClick={() => handleMercadoPago(true)}
                  disabled={procesando}
                  className={`w-full p-4 rounded-lg border-2 border-green-600 hover:border-green-400 hover:bg-green-500/10 transition-all duration-200 ${procesando ? 'opacity-50 cursor-not-allowed' : ''}`}
                  whileHover={{ scale: procesando ? 1 : 1.02 }}
                  whileTap={{ scale: procesando ? 1 : 0.98 }}
                >
                  <div className="flex items-center space-x-4">
                    ...
                    <div className="text-left">
                      <div className="text-white font-semibold">Mercado Pago Prueba</div>
                      <div className="text-gray-400 text-sm">Pago de prueba - Solo 0.2 centavos ARS</div>
                    </div>
                  </div>
                </motion.button>
