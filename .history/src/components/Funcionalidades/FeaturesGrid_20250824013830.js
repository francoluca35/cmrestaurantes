          ...
          {detailedFeatures.map((row, rowIndex) => (
            <div key={rowIndex} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
              {row.map((feature, featureIndex) => (
                <div 
                  key={featureIndex} 
                                     className={`p-4 sm:p-6 lg:p-4 rounded-lg border-2 ${feature.borderColor} bg-neutral-800/50 backdrop-blur-sm hover:bg-neutral-800/70 transition-all duration-300 transform hover:-translate-y-1 hover:shadow-lg`}
                >
