export default function FeaturesGrid() {
  const featureCategories = [
    ...
    { name: "mesas", icon: "🪑", color: "text-pink-500" },
    { name: "delivery", icon: "🚚", color: "text-purple-500" },
    { name: "e-commerce", icon: "🛒", color: "text-orange-500" },
    { name: "takeaway", icon: "📦", color: "text-pink-500" },
    { name: "self-service", icon: "🖥️", color: "text-orange-500" },
  ...
  return (
    <section className="py-16 sm:py-20 md:py-24 lg:py-28 xl:py-32 bg-gray-900">
        ...
        <div className="grid grid-cols-3 sm:grid-cols-5 lg:grid-cols-9 gap-4 sm:gap-6 mb-12 sm:mb-16 lg:mb-20">
          {featureCategories.map((category, index) => (
            <div key={index} className="text-center group">
              ...
              <p className="text-xs sm:text-sm lg:text-base text-gray-300 font-medium capitalize">
                {category.name}
              </p>
            </div>
          ))}
          ...
          {detailedFeatures.map((row, rowIndex) => (
            <div key={rowIndex} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
              {row.map((feature, featureIndex) => (
                <div 
                  key={featureIndex} 
                  className={`p-4 sm:p-6 lg:p-8 rounded-lg border-2 ${feature.borderColor} bg-gray-800/50 backdrop-blur-sm hover:bg-gray-800/70 transition-all duration-300 transform hover:-translate-y-1 hover:shadow-lg`}
                >
