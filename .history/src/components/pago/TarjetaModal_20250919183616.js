...
const TarjetaModal = ({ isOpen, onClose, onPagar, total, moneda = 'usd', tipoTarjeta = 'Tarjeta' }) => {
  ...
  // Detectar si es crédito o débito basado en el número
  const detectarCreditoDebito = (numero) => {
    const num = numero.replace(/\s/g, '')
    
    // Lógica para detectar crédito vs débito
    // En la práctica, esto sería más complejo, pero para demo usamos patrones
    if (num.length >= 13) {
      // Patrones que suelen ser crédito
      if (num.startsWith('4') && num.length === 16) return 'credito' // Visa crédito
      if ((num.startsWith('5') || num.startsWith('2')) && num.length === 16) return 'credito' // Mastercard crédito
      if (num.startsWith('3') && num.length === 15) return 'credito' // Amex crédito
      if (num.startsWith('6') && num.length === 16) return 'credito' // Discover crédito
      
      // Patrones que suelen ser débito
      if (num.startsWith('4') && num.length === 13) return 'debito' // Visa débito
      if (num.startsWith('5') && num.length === 13) return 'debito' // Mastercard débito
    }
    
    // Por defecto, asumimos crédito si no podemos determinar
    return 'credito'
  }
  ...
  useEffect(() => {
    if (total && moneda) {
      // Detectar automáticamente si es crédito o débito basado en el número de tarjeta
      const tipoPago = detectarCreditoDebito(datosTarjeta.numero)
      
      const calculoRecargos = calcularRecargos(total, tipoPago, moneda)
      const calculoLimites = verificarLimites(total, moneda)
      
      setRecargos(calculoRecargos)
      setLimites(calculoLimites)
    }
  }, [total, moneda, datosTarjeta.numero])

  // Resetear cuotas a 1 cuando es tarjeta de débito
  useEffect(() => {
    const tipo = detectarTipoTarjeta(datosTarjeta.numero)
    const esCredito = tipo === 'visa' || tipo === 'mastercard' || tipo === 'amex' || tipo === 'discover'
    
    if (!esCredito && datosTarjeta.numero) {
      setCuotas(1)
      setMostrarCuotas(false)
    } else if (esCredito && datosTarjeta.numero) {
          ...
          >
            ...
            <div className="p-6 border-b border-gray-200">
              <div className="flex items-center justify-between">
                <h2 className="text-2xl font-bold text-gray-900">
                  {tipoTarjetaDetectada ? 
                    (tipoTarjetaDetectada === 'visa' || tipoTarjetaDetectada === 'mastercard' || tipoTarjetaDetectada === 'amex' || tipoTarjetaDetectada === 'discover' ? 
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="space-y-4">
               ...
               {mostrarCuotas && (tipoTarjetaDetectada === 'visa' || tipoTarjetaDetectada === 'mastercard' || tipoTarjetaDetectada === 'amex' || tipoTarjetaDetectada === 'discover') && (
                 <motion.div
               ...
               {cuotas > 1 && (tipoTarjetaDetectada === 'visa' || tipoTarjetaDetectada === 'mastercard' || tipoTarjetaDetectada === 'amex' || tipoTarjetaDetectada === 'discover') && (
                 <motion.div
                ...
                >
                 ...
                 >
                   {procesando ? (
                     ...
                     (tipoTarjetaDetectada === 'visa' || tipoTarjetaDetectada === 'mastercard' || tipoTarjetaDetectada === 'amex' || tipoTarjetaDetectada === 'discover') && cuotas > 1
                       ? `Pagar ${cuotas} cuotas de $${((recargos ? recargos.total : total) / cuotas).toLocaleString()} ${moneda === 'usd' ? 'USD' : 'ARS'}`
