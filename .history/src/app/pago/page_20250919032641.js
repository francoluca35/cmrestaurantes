...
export default function PagoPage() {
  ...
  const handlePago = async (metodo) => {
    ...
    if (metodo === 'Tarjeta de Crédito') {
      // Aquí integrarías Stripe o tu procesador de tarjetas
      setTimeout(() => {
        setProcesando(false)
        alert(`¡Pago procesado con ${metodo}! Redirigiendo al dashboard...`)
      }, 2000)
    } else if (metodo === 'Mercado Pago') {
      // Aquí integrarías Mercado Pago
      setTimeout(() => {
        setProcesando(false)
        alert(`¡Pago procesado con ${metodo}! Redirigiendo al dashboard...`)
      }, 2000)
    } else if (metodo === 'PayPal') {
      // Aquí integrarías el botón de PayPal
      setTimeout(() => {
        setProcesando(false)
        alert(`¡Pago procesado con ${metodo}! Redirigiendo al dashboard...`)
      }, 2000)
  ...
  const calcularDesglose = () => {
    ...
    return {
      ...
      precioFinanzas,
      total: precioBase + precioDispositivosExtra + precioFinanzas
    }
  }
              ...
              <div className="border-t border-gray-600 pt-6">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="border-t border-gray-600 pt-2 flex justify-between font-semibold text-lg">
                    ...
                    <span className="text-green-400">${desglose.total}.00 USD</span>
                  </div>
                </div>
              </div>
            </motion.div>
                ...
                <motion.button
                  ...
                  className={`w-full p-4 rounded-lg border-2 transition-all duration-200 ${
                    formaPago === 'paypal' 
                      ...
                      : 'border-gray-600 hover:border-yellow-400 hover:bg-yellow-500/10'
                  } ${procesando ? 'opacity-50 cursor-not-allowed' : ''}`}
                  whileHover={{ scale: procesando ? 1 : 1.02 }}
                  whileTap={{ scale: procesando ? 1 : 0.98 }}
                >
                  <div className="flex items-center space-x-4">
                    <div className="w-12 h-8 bg-gradient-to-r from-yellow-500 to-orange-500 rounded flex items-center justify-center">
                      <svg className="w-6 h-6 text-white" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    <div className="text-left">
                      ...
                      <div className="text-gray-400 text-sm">Pago seguro con PayPal</div>
                    </div>
                  </div>
                </motion.button>
